...
public class SessionEnvironment
{
    ...
    public SessionInfoMock CreateSessionInfo(SessionObjectMock sessionObjectMock, DateTime openedAt,
        DateTime? expiredAt, string commandQuery = null)
    {
        return new SessionInfoMock
        {
            Data = sessionObjectMock,
            CommandQuery = commandQuery ?? "/myCommand",
            ExpiredAt = expiredAt,
            OpenedAt = openedAt,
            TelegramChatId = 1234,
            TelegramUserId = 5678
    ...
    public SessionInfoMockData CreateSessionInfoWithData(SessionObjectMock sessionObjectMock, DateTime openedAt,
        DateTime? expiredAt, string commandQuery = null)
    {
        return new SessionInfoMockData()
        {
            Data = sessionObjectMock,
            CommandQuery = commandQuery ?? "/myCommand",
            ExpiredAt = expiredAt,
    ...
    public (SessionInfoMockData, SessionObjectMock, DateTime, DateTime?) CreateSessionInfoWithData(string commandQuery = null)
    {
        var sessionObjectMock = CreateObject();
        var expiredAt = new DateTime(2023, 02, 01, 01, 02, 03);
        var openedAt = new DateTime(2022, 02, 01, 01, 02, 03);
        return (CreateSessionInfoWithData(sessionObjectMock, openedAt, expiredAt, commandQuery), sessionObjectMock, openedAt, expiredAt);
    }
}